...
img_orig = io.imread('peppers.jpg') #read the original image
...
mask_smoothed = np.array([[1,1,1],[1,1,1],[1,1,1]])/9 #according to the example in step1, mask of the smoothed image
mask_details = (mask_original - mask_smoothed)*9 #original - smoothed = details
...
for i in range(1, l-1):
    for j in range(1, c-1):
        ...
        V_smoothed = V * mask_smoothed
        V_details = V * mask_details
        V_sharpened = V * mask_sharpened
        
        img_smoothed[i, j] = np.sum(V_smoothed)
